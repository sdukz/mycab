...
class Database
    ...
    var $Host     = "localhost";        // Hostname of our MySQL server.
        var $Database = "mycab_mycab";         // Logical database name on that server.
        var $User     = "mycab_mycab";             // User and Password for login.
        var $Password = "jy4d4uRSazWA2DM";
   /* var $Host     = "localhost";        // Hostname of our MySQL server.
    var $Database = "MyCab";         // Logical database name on that server.
    ...
    function create_query($Data,$Table,$Type)
        {
        	...
        	switch ($Type) 
        	{
				case 'UPDATE':
					$Id = $Data['driver_id'];
					unset($Datt['driver_id']);
					foreach ($Data as $key => $value)
					{
						$str .= "`".$key."`='".$value."',";					
					}
					$str = rtrim($str,',');
					$query ="$Type $Table SET $str WHERE driver_id='$Id'";
					break;
			}
