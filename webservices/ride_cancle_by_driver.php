...
if(($DrivId !='' && $RideConId !=''))
{
   ...
   $aff_rw= mysql_affected_rows();
	...
	if($aff_rw > 0 && $aff_rw !='')
	{
	 ...
	 $loc ['loc-key'] = RIDE_CANCLE_BY_DRIVER; //Used for localization
	 $loc ['loc-args']= array($D['driver_name']);
	 
	 //send_notification('ride_cancle_by_driver','ride_cancle_by_driver#Driver -'.$D['driver_name'].' cancelled your ride.',$C['passanger_id']);
	 send_notification('ride_cancle_by_driver',$loc,$C['passanger_id']);
	 ...
	 $posts = array('message' => 'Yes','status'=>1);
	}
	else
	{
	 $posts = array('message' => 'No','status'=>0);
	}	
}
else
{
	$posts = array('message' => 'Driver Id And Ride Confirm Id Is Needed','status'=>0);
}
