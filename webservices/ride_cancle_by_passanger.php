...
if(($PassId !='' && $RideConId !=''))
{
   ...
   $aff_rw= mysql_affected_rows();
	...
	if($aff_rw > 0 && $aff_rw !='')
	{
	 ...
	 $loc ['loc-key'] = RIDE_CANCLE_BY_PASSANGER; //Used for localization
	 $loc ['loc-args']= array($P['passanger_name']);
	 
	 //send_notification('ride_cancle_by_passanger','ride_cancle_by_passanger#Passenger -'.$P['passanger_name'].' cancelled your ride.',$C['driver_id']);
	 send_notification('ride_cancle_by_passanger',$loc,$C['driver_id']);
