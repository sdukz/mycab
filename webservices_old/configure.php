...
{	
	...
	if($img != false)
	{
			...
			$ImagePath = 'http://'.$_SERVER["HTTP_HOST"].'/Mycab/avatar/'.$imagename.'.jpg';		
	}
	else
	{
...
FROM `driver` WHERE status=1 AND busy='No' AND balance !=0 AND balance >= $amt AND min_search_amount <= $amt AND max_search_amount >= $amt
...
{
	//$Data='';
	$query = "SELECT * FROM rate_review_passanger WHERE passanger_id='$PassId' AND haz_rude='Yes' ";
	$result= mysql_query($query);
	$count = mysql_num_rows($result);
...
{
	//$Data='';
	$query = "SELECT * FROM rate_review_passanger WHERE passanger_id='$PassId' AND spoiled_interier='Yes' ";
	$result= mysql_query($query);
	$count = mysql_num_rows($result);
...
{
	//$Data='';
	$query = "SELECT * FROM rate_review_passanger WHERE passanger_id='$PassId' AND did_not_come_to_car='Yes' ";
	$result= mysql_query($query);
	$count = mysql_num_rows($result);
...
{
	//$Data='';
	$query = "SELECT * FROM rate_review_passanger WHERE passanger_id='$PassId' AND cancel_order='Yes' ";
	$result= mysql_query($query);
	$count = mysql_num_rows($result);
